...
public class DraggableDetailsOverlayViewController: UIViewController {
    ...
    public func hide(animated: Bool, completion: (() -> Void)? = nil) {
        setVisible(false, animated: animated, initialAnchor: .top(offset: 0), completion: completion)
    }
    ...
    @objc private func handleDragGesture(_ recognizer: UIGestureRecognizer) {
        ...
        case .ended,
            ...
            if isSnapToAnchorsEnabled {
                ...
                } else if currentOffset != restOffset && (velocity.y <= 0 || (velocity.y > 0 && isContentScrollAtTop)) {
                    let isSpring = restOffset == cachedAnchorOffsets.first ? snapAnimationTopAnchorUseSpring : snapAnimationUseSpring
                    animateToOffset(restOffset, isSpring: isSpring, completion: nil)
                }
...
private extension DraggableDetailsOverlayViewController {
    ...
    private func offsetForAnchor(_ anchor: Anchor, topInset: CGFloat) -> CGFloat {
        ...
        case .middle(let containerHeight):
            let inset = view.bounds.height - containerHeight - topSafeAreaInset() - bottomSafeAreaInset()
    ...
    private func updateLayout(animated: Bool, forced: Bool) {
        ...
        guard newCurrentOffset.anchorOffset != draggableContainerShownTopConstraint.constant else {
            delegate?.draggableDetailsOverlayDidUpdatedLayout(self)
            return
        }
        if animated {
            animateToOffset(newCurrentOffset.anchorOffset, isSpring: false, completion: {
    ...
    private func setVisible(_ newVisible: Bool, animated: Bool, initialAnchor: Anchor, completion: (() -> Void)? = nil) {
        ...
        let animationCompletion = { (finished: Bool) -> Void in
            if !newVisible {
