...
public class PullToRefreshView: UIView {
    ...
    private let vibrationGenerator: VibrationGenerator?
    ...
    /// - parameter length: length of the view. It is height for vertical pull.
    ...
    /// - parameter enableHapticFeedback: If `true` transition into `.readyToTrigger` state will produce "light impact" haptic feedback.
    ///        Default value is `true`.
    public init(scrollView: UIScrollView,
        ...
        vibrationGenerator = enableHapticFeedback ? VibrationGenerator(newStyle: .light, oldStyle: nil) : nil
    ...
    private func setState(_ newValue: State, report: Bool, animated: Bool = true) {
        ...
        case .idle,
             .readyToTrigger:
            if newValue == .readyToTrigger {
                vibrationGenerator?.vibrate()
                vibrationGenerator?.prepare()
            }
